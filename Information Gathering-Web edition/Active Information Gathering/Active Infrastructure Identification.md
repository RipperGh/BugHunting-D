...
# HTTP Headers
```
GhostRipper@htb[/htb]$ curl -I "http://${TARGET}"
...
# WhatWeb
```
GhostRipper@htb[/htb]$ whatweb -a3 https://www.facebook.com -v
...
## Installing WafW00f
```
GhostRipper@htb[/htb]$ sudo apt install wafw00f -y
```
...
```
GhostRipper@htb[/htb]$ wafw00f -v https://www.tesla.com
...
# Installing Aquatone
```
GhostRipper@htb[/htb]$ sudo apt install golang chromium-driver
GhostRipper@htb[/htb]$ go install github.com/michenriksen/aquatone@latest
GhostRipper@htb[/htb]$ export PATH="$PATH":"$HOME/go/bin"
```
After this, we need to explore some options:

# Aquatone Options
```
GhostRipper@htb[/htb]$ aquatone --help
...
```
GhostRipper@htb[/htb]$ cat facebook_aquatone.txt | aquatone -out ./aquatone -screenshot-timeout 1000
...
![image](https://github.com/RipperGh/BugHunting-D/assets/165308866/8dedce39-319f-4910-99e1-3838e6fe1a32)

# Exercise
Target: 10.129.208.27

1) Question: + 1  What Apache version is running on app.inlanefreight.local? (Format: 0.0.0)

   Answer:2.4.41

   used curl target 

For these below, I had to do sudo vim /etc/hosts to add hosts with IP address target to be able to do whatweb -a3 or else i get an error of unreachable
